...
- **Per-Monitor DDC Health** — Each display keeps a persisted health record with writes sent, writes suppressed and their ratio, last successful read/write, consecutive failures, and measured round-trip latency. The Monitor tab shows the summary and can reset one display's counters; DDC support reports include the same lifetime accounting
