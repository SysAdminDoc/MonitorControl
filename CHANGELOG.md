...
- Changed: the deterministic no-hardware suite is validated against pinned Pester 5.9.0 and Pester 6.0.0. The application still runs on Windows PowerShell 5.1.
...
- Added: release builds now emit an offline-verifiable manifest with payload and ZIP SHA-256 hashes, source commit, runtime metadata, and a CycloneDX SBOM. Releases remain intentionally unsigned.
...
- Changed: the DDC compatibility report and its JSON variant list the effective input mapping per monitor, including where each value came from and the value that will actually be written. A user-typed input label is redacted with the same switch that redacts monitor friendly names.
...
- Added: the Profiles tab previews the selected profile before applying it. The preview covers matching records, missing settings, disconnected captured displays, unsupported values, and warnings recorded at capture time.
